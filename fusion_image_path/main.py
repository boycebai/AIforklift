...
Functions:  get_perspective_image, crop_arena, get_marker_color, check_wall,
            ...
            draw_grid, draw_forklift, draw_path, led_status, led_color,
...
# List which stores the encoded commands to be sent to the forklift
motion = []
turns_list = []
...
Output: Returns a perspective corrected and cropped image of the arena within the
        borders and list of the corner points of the this image
...
def get_perspective_image(frame):
	...
	qwer = cv2.polylines(frame, [approx], True, (0, 255, 0), 3)
	...
	# print (corner_points)
	# dst为透视并剪切变换后的地图，corner_points为透视前原图中四个角的坐标
...
Logic:  - A new image is created for mapping
        ...
        - At various wall postions, if a wall present a wall is drawn, if not, then
          a white path is drawn which is traverseble for the forklift
...
Input:  img -> Image where the forklift is present

Output: Returns the direction of the forklift
        1 -> North
        2 -> West
        3 -> South
        4 -> East

Logic:  - Finds the centroids of the two color stickers prsent on the forklift
        - Calculates the slope (tan inversere of dy/dx) of the line joining
          the two centroids
        - Returns the direction of the forklift based on the value of the slope
...
def get_direction(img):
	...
	o1 = [230, 210, 150]  # Orange sticker
	o2 = [255, 230, 180]
	p1 = [230, 170, 210]  # Pink sticker
	p2 = [255, 185, 230]
	...
	dx = x2 - x1
	dy = y2 - y1
	print("initial position bias: " ,dx, dy)

	# Calculte the angle
	rads = math.atan2(dy, dx)
	degs = math.degrees(rads)
	print("initial angle bias: " ,rads, degs)
...
Output: Returns a truth value wether the forklift is present or not

Logic:  - BGR image is converted to HSV image
        - Get the mean HSV values of the ROI image
        - Check if the forklift is present or not

Example Call: ret = check_position(img) // True if forklift is present
...
Output: The start grid is marked orange in the start position in the arena map
        Returns the start position, the co-ordinates and the start direction of the forklift

Logic:  - Create ROI images of the various all the start positions
        - Check at which position the forklift is prsent
        - Mark the start position in the arena map
        - Get the orientation of the forklift
...
def get_position(img, arena_grid):
	...
	if check_position(pos1):
		...
		# cv2.namedWindow("arena_grid", 0)
		# cv2.imshow("arena_grid", arena_grid)
		# cv2.waitKey(0)
		# cv2.destroyAllWindows()
...
Function Name: draw_forklift

Input:  img -> Image of the arena
        pos -> Start position of the forklift
        rot -> Start orientation of the forklift

Output: Draws the outline of the forklift in the arena and direction is indicated
        by a circle drawn towards the front of the forklift

Logic:  - Based on the start position, draw an outline circle of the forklift
        - Indicate the direction by a circle drawn towards the front of the forklift

Example Call: draw_forklift(img, pos, rot)

'''
def draw_forklift(img, pos, rot):
...
Output: The path is analysed and encoded into commands for the forklift

Logic:  - This function is used for encoding when the forklift is facing North
...
Logic:  - This function is used for encoding when the forklift is facing West
        - Based on the difference of x and y values of two consecutive set
          of co-ordinates in the list, the direction change is detected
        - Based on the new direction, the encoding function for that particular
          direction is then called and the path starting from that point is sent to that function
...
Logic:  - This function is used for encoding when the forklift is facing South
        - Based on the difference of x and y values of two consecutive set
          of co-ordinates in the list, the direction change is detected
        - Based on the new direction, the encoding function for that particular
          direction is then called and the path starting from that point is sent to that function
...
Logic:  - This function is used for encoding when the forklift is facing East
        - Based on the difference of x and y values of two consecutive set
          of co-ordinates in the list, the direction change is detected
        - Based on the new direction, the encoding function for that particular
          direction is then called and the path starting from that point is sent to that function
...
Input:  path -> The master path list
        rotation -> The start oreintation of the forklift

Output: The entire path of the forklift in the arena is encoded into commands
        which is to be sent to the forklift
...
Logic:  - If offset is gretaer than 0, turn the forklift left
        - If offset is lesser than 0, turn the forklift right
...
Output: The encoded commands are sent to the forklift one by one
        and feedback is done at every turn of the forklift

Logic:  - Serial communication is used to send the command one by one
        - The forklift and the computer are first synced before communication
        - When a data of '1' is recieved from the forklift, the next command is sent
        - When a data of '2' is recieved from the forklift, feedback is processed (after every turn)
...
def Wifi_communication():
	...
	print('Searching for forklift... (Ctrl+C to abort communication at any time)\n')

	# Sync forklift and computer
	while True:
		if (xbee.inWaiting() > 0):
			start_flag = xbee.read()
			if start_flag is '0':
				print('forklift found!')
				xbee.write(chr(255))
				break
...
Logic:  - Captures the image of the arena
        ...
        - Shows the webcam input image and arena map with the planned path and forklift position
...
if master_mode is 1:
    ...
    cv2.namedWindow("arena_input", 0)
    cv2.imshow('arena_input',arena_input)
    arena_crop = arena.copy()

elif master_mode is 0:
    # arena_input = cv2.imread('/home/boyce/Desktop/BigData/canku/image/0828/Autonomous-Indoor-Navigation-using-Image-Processing-master/images/3.jpg') ## INPUT IMAGE ##
    arena_input = cv2.imread('./image/1.jpg')  ## INPUT IMAGE ##
    # arena_input = cv2.imread('/home/boyce/Desktop/BigData/canku/image/0828/Autonomous-Indoor-Navigation-using-Image-Processing-master/images/2.jpg')  ## INPUT IMAGE ##
    #arena_input1 = np.array(arena_input)
    #print(arena_input1.shape)
    cv2.namedWindow("arena_input", 0)
    cv2.imshow('arena_input', arena_input)
    arena, corners = get_perspective_image(arena_input)
    #cv2.waitKey(0)

else: arena = cv2.imread('./image/1.jpg')
...
# Get the forklift position and orientation
pos, pos_cord, rot = get_position(img, arena_grid)
print("#################")
if pos is 0:
    warnings.warn('\n\nforklift Not Found!\nStart Position: 2')
else:
    print ('\nforklift Position:',pos)
    print ('forklift Direction:',direction_dict[rot])
...
# Draw forklift
if pos is not 0: draw_forklift(0, pos, rot)

# Show Images
cv2.namedWindow("Arena Live", 0)
cv2.imshow('Arena Live', arena)

cv2.namedWindow("Master Path", 0)
...
if master_mode is 1:
	print('I can communication with Host!')
# Wifi_communication()
...
if master_mode is 1 and success_flag:
	score_b = 0
	score_c = int(input('Provisions Delivered: '))
	score_w = int(input('Provisions Delivered Incorrectly: '))
	score_p = int(input('Obstacles Displaced: '))
